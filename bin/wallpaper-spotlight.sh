...
check_dependencies() {
  ...
  if [ -n "$WAYLAND_DISPLAY" ] || [ -n "$DISPLAY" ]; then
    if ! pgrep -x "awww-daemon" >/dev/null; then
      log_message "WARNING: awww-daemon not running. Start it in your session (e.g., 'awww-daemon')."
    fi
  fi
}
...
# Set wallpaper with awww (if session available)
set_wallpaper() {
  local image_file="$1"
  ...
  if ! pgrep -x "awww-daemon" >/dev/null; then
    log_message "ERROR: awww-daemon is not running."
    echo "$image_file" >"$CURRENT_WALLPAPER"
    return 1
  fi

  if ! awww query &>/dev/null; then
    log_message "ERROR: awww is not initialized."
    echo "$image_file" >"$CURRENT_WALLPAPER"
    return 1
  fi

  log_message "Setting wallpaper: $(basename "$image_file")"
  if awww img "$image_file" --transition-type grow --transition-duration 1; then
    echo "$image_file" >"$CURRENT_WALLPAPER"
    log_message "Wallpaper set successfully."
    return 0
  else
    log_message "ERROR: Failed to set wallpaper with awww"
    echo "$image_file" >"$CURRENT_WALLPAPER"
    return 1
  fi
}
...
status)
  ...
  if pgrep -x "awww-daemon" >/dev/null; then
    echo "awww-daemon: Running"
    if awww query &>/dev/null; then
      echo "awww: Initialized"
    else
      echo "awww: Not initialized"
    fi
  else
